...
/**
 * ASCII glitch-ripple text. Scrambles characters in a wave that radiates from
 * the hover point. `autoStart` fires the ripple on its own (used on the
 * loading HUD so it glitches in without needing a hover).
 */
export function AsciiGlitchRipple({
  ...
  useEffect(() => {
    ...
    // Auto-glitch (for the loading HUD — no hover needed)
    let autoT0, autoTimer;
    if (autoStart) {
      const fire = () => {
        stateRef.current.cursorPos = 0;
        startWave();
      };
      autoT0 = setTimeout(fire, 250);
      autoTimer = setInterval(fire, autoInterval);
    }
    ...
    return () => {
      ...
      if (autoT0) clearTimeout(autoT0);
      if (autoTimer) clearInterval(autoTimer);
      if (stateRef.current.animId) cancelAnimationFrame(stateRef.current.animId);
    };
  }, [children, autoStart, autoInterval]);

  return (
    <Component
      ref={elRef}
      className={cn(
        'select-none relative inline-block transition-colors duration-200',
        className
      )}
      {...props}
    />
